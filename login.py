...
class Login_window:
    def __init__(self,root):
        ...
        self.left=ImageTk.PhotoImage(file="img/side3.jpg")
        left=Label(self.root,image=self.left,text='Student Result \n Management System',compound='center',fg='white',font=("goudy old style",24,"bold")).place(x=80,y=100,width=400,height=500)
        
        # --------------frames---------------------------------
        login_frame=Frame(self.root,bg='white',)
        login_frame.place(x=480,y=100,width=600,height=500)
        
        title=Label(login_frame,text="LOGIN HERE",font=("times new roman",30,"bold"),bg="white",fg="#08A3D2").place(x=60,y=50)
        # ------------------------------------------------------------
        email=Label(login_frame,text="EMAIL ADDRESS",font=("times new roman",18,"bold"),bg="white",fg="gray").place(x=60,y=150)
        self.txt_email=Entry(login_frame,font=("times new roman",18),bg="lightgray",highlightbackground = "black", highlightcolor= "black",highlightthickness=1)
        self.txt_email.place(x=60,y=180,width=350,height=35)
        
        
        passs=Label(login_frame,text="PASSWORD",font=("times new roman",18,"bold"),bg="white",fg="gray").place(x=60,y=250)
        self.txt_passs=Entry(login_frame,font=("times new roman",18),bg="lightgray",show="*",highlightbackground = "black", highlightcolor= "black",highlightthickness=1)
        self.txt_passs.place(x=60,y=280,width=350,height=35)
        
        btn_reg=Button(login_frame,text="Register new Account ?",command=self.reg_window,font=("times new roman",14),bg='white',bd=0,fg='#B00857',cursor='hand2').place(x=60,y=320)
        btn_forget=Button(login_frame,text="Forget Password ?",command=self.forget_pass_window,font=("times new roman",14),bg='white',bd=0,fg='red',cursor='hand2').place(x=260,y=320)
        
        btn_login=Button(login_frame,text="Login",font=("times new roman",20,"bold"),fg='white',bg='#B00857',cursor='hand2',command=self.login).place(x=60,y=380,width=180,height=40)
    ...
    def forpass(self):
            ...
            title=Label(self.root2,text="Forget Password",font=("times new roman",20,"bold"),bg='white',fg='red').place(x=0,y=10,relwidth=1)
            ...
            new_pass=Label(self.root2,text="New Password",font=("times new roman",15,"bold"),bg="white",fg="gray").place(x=80,y=260)
            self.txt_new_pass=Entry(self.root2,font=("times new roman",15),bg="lightgray")
            self.txt_new_pass.place(x=80,y=290,width=250) 
            
            btn_change_pass=Button(self.root2,text="Reset Password",command=self.forget_pass,bg='green',fg='white',font=("times new roman",15,"bold")).place(x=130,y=340)
    ...
    def login(self):
        if self.txt_email.get()=="" or self.txt_passs.get()=="":
            ...
            try:
                con=sqlite3.connect(database="srms.db")
                cur=con.cursor()
                cur.execute("select * from employee where email=? and password=?",(self.txt_email.get(),self.txt_passs.get()))
                row=cur.fetchone()
                if row==None:
